...
def remove_outliers(df: pd.DataFrame) -> pd.DataFrame:
   ...
   # Handling outliers in YearAdded column with minimum year within the last 30 years
    if 'yearadded' in df.columns and df['yearadded'].dtype in ['int64', 'float64']:
        current_year = pd.Timestamp.now().year
        min_year = current_year - 30
        Q1 = df['yearadded'].quantile(0.25)
        Q3 = df['yearadded'].quantile(0.75)
        IQR = Q3 - Q1
        lower_bound = max(Q1 - 1.5 * IQR, min_year)  # Ensure earliest year is within the last 30 years
        upper_bound = Q3 + 1.5 * IQR
...
def standardize_formats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'supplier' in df.columns:
        ...
        df['supplier'] = df['supplier'].replace({'Nitendo': 'Nintendo'})
